...
def test_custom_sql_type_mapping_file(temp_dir):
...
DEFAULT: Any
"""
    custom_yaml_file = os.path.join(temp_dir, "custom_types.yaml")
...
"""
    ...
    assert parser.get_python_type("CUSTOM_INT") == "int"
    assert parser.get_python_type("CUSTOM_STRING") == "str"
    assert parser.get_python_type("CUSTOM_FLOAT") == "float"

    # Verify unknown types fall back to DEFAULT
    assert parser.get_python_type("UNKNOWN_TYPE") == "Any"


def test_default_sql_type_mapping_when_not_specified(temp_dir):
...
def test_custom_sql_type_mapping_in_code_generator(temp_dir):
...
"""
    ...
    with temp_sql_files(sql_content, schema_content) as (sql_file, schema_file):
        ...
        assert generator._schema_parser.get_python_type("CUSTOM_INT") == "int"
        assert generator._schema_parser.get_python_type("CUSTOM_STRING") == "str"
